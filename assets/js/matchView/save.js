            ...
            $.ajax({
                ...
                success: function(data2, jqXHR) {
                    ...
                    for(var playercount = 0; playercount < data.data.players.all_players.length; playercount++) {
                        ...
                        deathsArray.push(data.data.players.all_players[playercount].stats.deaths)
                    }
                    var kdArray = [];
                    var playerArray = [];
                    for(var count = 0; count < killsArray.length; count++) {
                        kdArray.push(killsArray[count] / deathsArray[count])
                        playerArray.push(data.data.players.all_players[count].name + "#" + data.data.players.all_players[count].tag + " " + killsArray[count] / deathsArray[count])
                    }
                    var sortedKDArray = kdArray.sort((a, b) => b - a)
